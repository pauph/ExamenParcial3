...
int main() {
    ...
    for (filaActual = 0; filaActual <= id && fgets(buffer, sizeof(buffer), archivoentrada); ++filaActual) {
    }

    for (filaActual = 0; filaActual < maxfilas && fgets(buffer, sizeof(buffer), archivoentrada); ++filaActual) {
        char *token = strtok(buffer, ",");
        int columna = 0;
        while (token && columna < maxcolumnas) {
